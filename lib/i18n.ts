...
> = {
  en: {
    home: {
      heroTitle: "Vibe-coded? Get your security score.",
      heroSubtitle:
    ...
    result: {
      ...
      scoreCriteriaSecurity: "Security: GitHub + OpenSSF Scorecard; Snyk issues (if configured) reduce score.",
      scoreCriteriaQuality: "Code quality: description, stars, repo size, issues.",
      scoreCriteriaDeps: "Dependency risk: package.json dep count; Socket.dev score blended when configured.",
  ...
  ko: {
    home: {
      heroTitle: "Vibe로 코딩했나요? 보안 점수를 확인하세요.",
      heroSubtitle:
    ...
    result: {
      ...
      scoreCriteriaSecurity: "보안: GitHub + OpenSSF Scorecard; Snyk 이슈(설정 시) 반영.",
      scoreCriteriaQuality: "코드 품질: 설명, 스타 수, 저장소 크기, 이슈 사용.",
      scoreCriteriaDeps: "의존성 위험: package.json 개수; Socket.dev 설정 시 점수 반영.",
      detailsTitle: "검사한 항목",
      ...
      details: {
        license: { label: "라이선스", tip: "LICENSE 파일을 추가해 다른 사람이 어떻게 사용할 수 있는지 알려주세요 (예: MIT, Apache 2.0).", plain: "쉽게 말하면: 이 코드를 다른 사람이 써도 되는지, 어떻게 써야 하는지 적어 둔 문서예요. 없으면 ‘무단 사용 금지’로 해석될 수 있어요." },
        recent_activity: { label: "최근 업데이트", tip: "저장소를 주기적으로 업데이트하세요. 보안 이슈를 수정하고 의존성을 갱신하세요.", plain: "쉽게 말하면: 마지막으로 코드가 수정된 시점이에요. 오래됐으면 프로젝트가 방치됐거나 낡았을 수 있어요." },
        ...
        lock_file: { label: "락 파일", tip: "package-lock.json(또는 yarn.lock, pnpm-lock.yaml)을 커밋하면 모두 같은 의존성 버전을 쓰게 되어 공급망 위험이 줄어듭니다.", plain: "쉽게 말하면: 쓰는 라이브러리 버전을 고정해 둔 파일이에요. 없으면 환경마다 다른 버전이 설치돼서 ‘우리 쪽에선 되는데’ 같은 일이 생길 수 있어요." },
  ...
  ja: {
    home: {
      heroTitle: "Vibeでコーディングしましたか？セキュリティスコアを確認。",
      heroSubtitle:
    ...
    result: {
      ...
      scoreCriteriaSecurity: "セキュリティ: GitHub + OpenSSF Scorecard。Snyk(設定時)で減点.",
      scoreCriteriaQuality: "品質: 説明、スター数、リポジトリサイズ、Issues.",
      scoreCriteriaDeps: "依存関係: package.json本数。Socket.dev設定時はスコアをブレンド.",
